...
func (s *service) Create(input *model.Created) (*model.Base, error) {
	...
	output.StockEquipmentID = util.GenerateUUID()
...
func (s *service) ListByStockID(stockID string) (output []*model.Base, err error) {
	fields, err := s.Entity.ListByStockID(stockID)
	if err != nil {
		log.Error(err)
		return nil, err
	}
...
func (s *service) Delete(input *model.Updated) (err error) {
	field, err := s.Entity.GetByID(&model.Field{StockEquipmentID: input.StockEquipmentID})
	if err != nil {
		log.Error(err)
		return err
	}
